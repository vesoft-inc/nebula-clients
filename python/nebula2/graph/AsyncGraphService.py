...
class Client(Iface):
  ...
  def recv_executeJson(self, iprot, mtype, rseqid):
    try:
        fut = self._futures.pop(rseqid)
    except KeyError:
        return   # request timed out
    ...
    if result.success != None:
      fut.set_result(result.success)
      return
    fut.set_exception(TApplicationException(TApplicationException.MISSING_RESULT, "executeJson failed: unknown result"))
    return
...
class Processor(Iface, TProcessor):
  ...
  @process_method(executeJson_args, oneway=False, asyncio=True)
  def process_executeJson(self, args, handler_ctx, seqid, oprot, fn_name):
    result = executeJson_result()
    if should_run_on_thread(self._handler.executeJson):
      fut = self._loop.run_in_executor(None, self._handler.executeJson, args.sessionId, args.stmt)
    else:
      fut = call_as_future(self._handler.executeJson, self._loop, args.sessionId, args.stmt)
    fut.add_done_callback(lambda f: write_results_after_future(result, self._event_handler, handler_ctx, seqid, oprot, fn_name, {}, f))
    return fut
...
class ContextProcessor(ContextIface, TProcessor):
  ...
  def process_executeJson(self, args, handler_ctx, seqid, oprot, fn_name):
    result = executeJson_result()
    if should_run_on_thread(self._handler.executeJson):
      fut = self._loop.run_in_executor(None, self._handler.executeJson, handler_ctx, args.sessionId, args.stmt)
    else:
      fut = call_as_future(self._handler.executeJson, self._loop, handler_ctx, args.sessionId, args.stmt)
    fut.add_done_callback(lambda f: write_results_after_future(result, self._event_handler, handler_ctx, seqid, oprot, fn_name, {}, f))
    return fut
