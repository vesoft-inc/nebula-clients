...
var testPoolConfig = conf.GetDefaultConf(log)
...
func TestPool_SingleHost(t *testing.T) {
	...
	testPoolConfig = conf.NewPoolConf(TimeOut, IdleTime, MaxConnPoolSize, MinConnPoolSize, log)
...
func TestPool_MultiHosts(t *testing.T) {
	...
	multiHostsConfig := conf.NewPoolConf(TimeOut, IdleTime, MaxConnPoolSize, MinConnPoolSize, log)

	// Initialize connectin pool
	err = pool.InitPool(hostList, multiHostsConfig, log)
	if err != nil {
...
func TestReconnect(t *testing.T) {
	...
	timeoutConfig := conf.NewPoolConf(TimeOut, IdleTime, MaxConnPoolSize, MinConnPoolSize, log)

	// Initialize connectin pool
	err := pool.InitPool(hostList, timeoutConfig, log)
	if err != nil {
		t.Fatalf("Fail to initialize the connection pool, host: %s, port: %d, %s", address, port, err.Error())
	}
