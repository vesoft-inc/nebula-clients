...
func (valWarp ValueWrapper) String() string {
	...
	if value.IsSetNVal() {
		...
		date := value.GetDVal()
		return fmt.Sprintf("%d-%2d-%2d", date.Year, date.Month, date.Day)
	} else if value.IsSetTVal() { // Time HH:MM:SS.MS
		time := value.GetTVal()
		return fmt.Sprintf("%2d:%2d:%2d.%3d", time.Hour, time.Minute, time.Sec, time.Microsec)
	} else if value.IsSetDtVal() { // DateTime yyyy-mm-ddTHH:MM:SS.MS  TODO: add time zone
		dateTime := value.GetDtVal()
		return fmt.Sprintf("%d-%2d-%2dT%2d:%2d:%2d.%3d",
